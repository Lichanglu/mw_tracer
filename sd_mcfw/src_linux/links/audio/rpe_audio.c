...
Void* Audio_setAacEncParam(Void *ctxMem, AENC_CREATE_PARAMS_S *pPrm,int sampleRate)
{
   ...
   // memset(ctx, 0, sizeof(encContextDsp));
	...
	AUDIO_INFO_PRINT(("encParams=%p\n",encParams));
    /*--------------------------------------------------------------------*/
    /* Set AAC encoder create time parameters                             */
    /*--------------------------------------------------------------------*/
    encParams->sampleRate     = pPrm->sampleRate=sampleRate;
	...
	*(ctx->cmdId) = XDM_SETPARAMS;
	...
	AUDIO_INFO_PRINT(("2encParams=%p\n",encParams));
	
    status = Rpe_control (ctx->encControlCallDesc);
	printf("encParams->inputBitsPerSample====%d,status=%d=%d\n",encParams->sampleRate,status,RPE_S_SUCCESS);
