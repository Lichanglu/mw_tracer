...
Void IpcFramesInLink_notifyCb(OSA_TskHndl * pTsk)
{
    ...
    else
    {
        ...
        OSA_printf("IPC_FRAMESINLINK CBFXN:!WARNING!.Commands not being processed by link."
...
Void *IpcFramesInLink_periodicTaskFxn(Void * prm)
{
    ...
    while (FALSE == pObj->prd.exitThread)
    {
        ...
        else
        {
            ...
            OSA_printf("IPC_FRAMESINLINK PERIODTSK:!WARNING!.Commands not being processed by link."
                       "TimeSinceLastAlloc:%d,TimeSinceLastFree:%d",
                       (curTime - pObj->stats.lastGetBufTime),
                       (curTime - pObj->stats.lastFreeBufTime));
        }
...
//	printf("IpcFramesInLink_periodicTaskFxn = %p\n",IpcFramesInLink_periodicTaskFxn);
	printf_pthread_create(__FILE__, ("IpcFramesInLink_periodicTaskFxn"));
...
Int32 IpcFramesInLink_init()
{
	...
	printf_pthread_create(__FILE__, ("IpcFramesInLink_tskMain"));
