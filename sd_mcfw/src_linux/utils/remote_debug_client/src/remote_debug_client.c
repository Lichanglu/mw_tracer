...
#define ZLOG_BIOS_LOG_CONFIG			("/usr/local/reach/dvr_rdk/ti816x/bios_log.conf")
#define ZLOG_BIOS_LOG_CAT				("bios_cat")
...
STATIC int RemoteDebug_getChar(unsigned int coreObjPhysBaseAddr, unsigned int coreId,
											char expectedChar, unsigned int timeout,
											zlog_category_t *bios_cat)
{
    volatile RemoteDebug_CoreObj *pCoreObj;
    volatile int value;
    char receivedChar;
    ...
    while(1)
    {
        ...
        if(value & REMOTE_DEBUG_FLAG_TYPE_CHAR)
        {
            ...
            {
            	if(bios_cat != NULL){
					zlog_info(bios_cat, "555 %s Received character '%c'\n", gRemoteDebug_corePrefix[coreId], receivedChar );	
				}
				else {
              		printf("555 %s Received character '%c'\n", gRemoteDebug_corePrefix[coreId], receivedChar );
				}
              break;
            }
        }
...
int RemoteDebug_clientRun(unsigned int coreObjPhysBaseAddr, zlog_category_t *bios_cat)
{
  unsigned int coreId;
  int numBytes, status;
  unsigned int doWait;
...
int main(int argc, char **argv)
{
  ...
  if(remoteDebugShmPhysAddr==0 || (remoteDebugShmPhysAddr & 0xFFF))
  {
    ...
    exit(0);
  	...
  	zlog_init(ZLOG_BIOS_LOG_CONFIG);

	bios_cat = zlog_get_category(ZLOG_BIOS_LOG_CAT);
	if(bios_cat == NULL){
		printf("get bios_cat fail\n");
	}
  ...
  if(argc==5 || argc==4)
  {
    ...
    {
      RemoteDebug_getChar(remoteDebugShmPhysAddr, coreId,
	  						argv[3][0], (unsigned int)-1, bios_cat);
    }
  }
  else
  {
    RemoteDebug_clientRun(remoteDebugShmPhysAddr, bios_cat);
  }
