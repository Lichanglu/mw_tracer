...
int OSA_msgqSendMsgEx(OSA_MsgqHndl *to, OSA_MsgqHndl *from, Uint16 cmd, void *prm, Uint16 msgFlags, OSA_MsgHndl **msg,Uint32 timeout)
{
  int status;
  OSA_MsgHndl *data;
  ...
  if(data==NULL)
    return OSA_EFAIL;
  status = OSA_msgqSend(to, data, timeout);
  if(status==OSA_SOK) 
  {
    if(msg!=NULL)
      *msg = data;
