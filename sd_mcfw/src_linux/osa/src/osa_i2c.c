...
int OSA_i2cRead16(OSA_I2cHndl *hndl, Uint16 devAddr, Uint8 *reg, Uint8 *value, Uint32 count)
{
    int i, j;
    int status = 0;
    struct i2c_msg * msgs = NULL;
...
int OSA_i2cWrite16(OSA_I2cHndl *hndl, Uint16 devAddr,  Uint8 *reg, Uint8 *value, Uint32 count)
{
    int i,j;
    unsigned char * bufAddr;
    int status = 0;
    ...
    msgs = (struct i2c_msg *) malloc(sizeof(struct i2c_msg));
    ...
    bufAddr = (unsigned char *) malloc(sizeof(unsigned char) * 3);
