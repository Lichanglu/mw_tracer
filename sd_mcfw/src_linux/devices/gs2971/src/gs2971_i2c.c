...
static int gGS2971_Lps = -1;
static int gGS2971_Mode=-1;
static int gGS2971_TMDS = -1;
static int gGS2971_Hpv = 0;
static int gGS2971_Vps = 0;
...
static Vps_Gs2971_sav_eav_vbi  gs2971_SavEavVbi[DEVICE_STD_REACH_LAST] = {
...
static Vps_Gs2971_InMode ArgMode_GS2971[DEVICE_STD_REACH_LAST] = {
...
/* ========================================================================== */
UInt16 Device_gs2971_Read16_CPLD(Device_Gs2971Obj * pObj, UInt8 RegAddr)
{
	Int32 status = 0;
	UInt8 regAddr[1]={0};
	UInt8 regValue[2]={0};
...
UInt16 Device_gs2971_Write16_CPLD(Device_Gs2971Obj * pObj, UInt8 RegAddr, UInt16 RegVal)
{
	Int32 status = 0;
	UInt8 regAddr[1]={0};
	UInt8 regValue[1]={0};
	...
	status = OSA_CPLD_i2cWrite16 (&gDevice_gs2971CommonObj.i2cHandle, 0, regAddr, regValue, 2 );

	return status;
}

UInt8 Device_gs2971_Read8(Device_Gs2971Obj * pObj, UInt8 RegAddr)
{
	Int32 status = 0;
	UInt8 regAddr[1]={0};
	UInt8 regValue[1]={0};
	...
	regAddr[0] = RegAddr;
	status = OSA_i2cRead8 (&gDevice_gs2971CommonObj.i2cHandle, GS2971_IIC_SLAVE_ADDR, regAddr, regValue, 1);

	return regValue[0];
}

Int32 Device_gs2971_Write8(Device_Gs2971Obj * pObj, UInt8 RegAddr, UInt8 RegVal)
{
	Int32 status = 0;
	UInt8 regAddr[1]={0};
	UInt8 regValue[1]={0};
	...
	regAddr[0] = RegAddr;
	regValue[0] = RegVal;
	status = OSA_i2cWrite8 (&gDevice_gs2971CommonObj.i2cHandle, GS2971_IIC_SLAVE_ADDR, regAddr, regValue, 1 );

	return status;
}

static Int32 Device_gs2971Config(Device_Gs2971Obj *pObj, int inMode, int tmds)
{
	Int32 	retVal = 0;
	return retVal;
}

static Int32 Device_gs2971GetResolution(Device_Gs2971Obj *pObj)
{
	Int32 	hsfq = 0, linenum=0;
	int i=0, inMode=0;;
	unsigned short cpld_version=0;
	hsfq  = Device_gs2971_Read16_CPLD(pObj, 0x0);
	linenum  = Device_gs2971_Read16_CPLD(pObj, 0x2);
	cpld_version = Device_gs2971_Read16_CPLD(pObj, 15*2);

	for(i = 0;i<DEVICE_STD_REACH_LAST;i++){
		if(((hsfq>gs2971_gLineNum[i].lps-10)&&(hsfq<gs2971_gLineNum[i].lps+10))&&((linenum>gs2971_gLineNum[i].hpv-4)&&(linenum<gs2971_gLineNum[i].hpv+4))){
			break;
		}
	...
	if(i == DEVICE_STD_REACH_LAST){
		for(i = 0;i<DEVICE_STD_REACH_LAST;i++){
			if(((hsfq>gs2971_gLineNum[i].lps-10)&&(hsfq<gs2971_gLineNum[i].lps+10))&&((linenum>gs2971_gLineNum[i].hpv-10)&&(linenum<gs2971_gLineNum[i].hpv+10))){
				break;
			}
	    	}
	}
	...
	gGS2971_Lps = hsfq;
	gGS2971_Mode = inMode;
	g_fpga_vesion = cpld_version;

	if(gGS2971_Mode != -1){
		printf("###########################################\n");
		printf("Get GS2971 Resolution!\n");
		printf("gGS2971_std = %s \n",ArgMode_GS2971[gGS2971_Mode].name);
		printf("gGS2971_hpv = %d gGS2971_lps = %d \n",gGS2971_Hpv,gGS2971_Lps);
		printf("gGS2971_Mode = %d \n",gGS2971_Mode);
		printf("FPGA Version = 0x%x \n",cpld_version);
		printf("###########################################\n");
	}else{
		printf("###########################################\n");
		printf("Get GS2971 Resolution!\n");
		printf("gGS2971_std = unknow! \n");
		printf("gGS2971_hpv = %d gGS2971_lps = %d \n",gGS2971_Hpv,gGS2971_Lps);
		printf("gGS2971_Mode = %d \n",gGS2971_Mode);
		printf("FPGA Version = 0x%x \n",cpld_version);
		printf("###########################################\n");
...
Int32 Device_gs2971GetExternInformation(Device_Gs2971Obj * pObj,
							Device_VideoDecoderExternInforms* extenInfor)
{
	Int32 status = 0;
	
	memcpy(extenInfor->DeviceName, "gs2971", sizeof(extenInfor->DeviceName));
	extenInfor->ModeID = gGS2971_Mode;
	extenInfor->SignalTmds = gGS2971_TMDS;
	extenInfor->SignalHpv = gGS2971_Hpv;
	extenInfor->SignalFreq = gGS2971_Vps;
	extenInfor->SignalYPbPr = 0;
	extenInfor->SignalHsfqFpga = gGS2971_Lps;
	extenInfor->SignalLinenumFpga = gGS2971_Hpv;
...
Int32 Device_gs2971Start ( Device_Gs2971Obj * pObj )
{
    ...
    status = Device_gs2971OutputEnable ( pObj, TRUE );
...
Int32 Device_gs2971Stop ( Device_Gs2971Obj * pObj )
{
    ...
    status = Device_gs2971OutputEnable ( pObj, FALSE );
                                     ...
                                     VCAP_VIDEO_SOURCE_CH_STATUS_S     * pStatus )
{
	...
	Device_gs2971GetResolution(pObj);
	if(gGS2971_Mode != -1){
                            ...
                            Device_VideoDecoderRegRdWrParams * pPrm )
{
    ...
    status = OSA_i2cWrite8 (&gDevice_gs2971CommonObj.i2cHandle, pCreateArgs->deviceI2cAddr[0], pPrm->regAddr, pPrm->regValue8, pPrm->numRegs );
                           ...
                           Device_VideoDecoderRegRdWrParams * pPrm )
{
    ...
    status = OSA_i2cRead8 (&gDevice_gs2971CommonObj.i2cHandle, pCreateArgs->deviceI2cAddr[0], pPrm->regAddr, pPrm->regValue8, pPrm->numRegs);
