...
printf("#####################Device_tvp5158SetVideoMode############# \n");
    if ( pPrm == NULL )
        return -1;
...
printf("#####################pPrm->videoCaptureMode=%x############# \n",pPrm->videoCaptureMode);	
        /*
         * select mux mode, vdet mode based on 'videoCaptureMode'
         */
     //    pPrm->videoCaptureMode=DEVICE_CAPT_VIDEO_CAPTURE_MODE_MULTI_CH_PIXEL_MUX_EMBEDDED_SYNC;
	...
	printf("#############reg 0xb1 =%x ################# \n",regValue[numRegs]);
         ...
         * apply the settings
         */
        status = OSA_i2cWrite8 (&gDevice_tvp5158CommonObj.i2cHandle, TVP5158_I2C_ADDR, regAddr, regValue, numRegs);


        regAddr[0] = 0x16;
        regValue[0] = 0x60;  /* No mask for autoswitch  */

      //  status = OSA_i2cWrite8 (&gDevice_tvp5158CommonObj.i2cHandle, TVP5158_I2C_ADDR, regAddr, regValue, 1);
        if ( status < 0 )
            return -1;
...
Int32 Device_tvp5158SetExtendedSettings( Device_Tvp5158Obj * pObj)
{
    ...
    for ( devId = pCreateArgs->numDevicesAtPort - 1; devId >= 0; devId-- )
    {
        ...
        status = OSA_i2cRead8 (&gDevice_tvp5158CommonObj.i2cHandle, TVP5158_I2C_ADDR, regAddr, regValue, numRegs);
...
Int32 Device_tvp5158PatchDownload ( Device_Tvp5158Obj * pObj, UInt32 devId )
{
    ...
    while ( patchSize )
    {
        ...
        status = OSA_i2cWrite8 (&gDevice_tvp5158CommonObj.i2cHandle, TVP5158_I2C_ADDR, regAddr, patchAddr, wrSize );
...
Device_tvp5158SetVideoMode (  pObj, &VideoModeParams);
 Device_tvp5158OutputEnable (  pObj, 1 )  ;                             
...
for(i=0;i<0x100;i++){
    ...
    status = OSA_i2cRead8 (&gDevice_tvp5158CommonObj.i2cHandle, TVP5158_I2C_ADDR, regAddr, regValue, 1);

printf("##############reg regAddr[%x]=%x ############### \n",i,regValue[0]);	
                            ...
                            Device_VideoDecoderRegRdWrParams * pPrm )
{
    ...
    status = OSA_i2cWrite8 (&gDevice_tvp5158CommonObj.i2cHandle, TVP5158_I2C_ADDR, pPrm->regAddr, pPrm->regValue8, pPrm->numRegs );
                           ...
                           Device_VideoDecoderRegRdWrParams * pPrm )
{
    ...
    status = OSA_i2cRead8 (&gDevice_tvp5158CommonObj.i2cHandle, TVP5158_I2C_ADDR, pPrm->regAddr, pPrm->regValue8, pPrm->numRegs);
