                               ...
                               Bool isTiled)
{
    ...
    doLineXfer = FALSE;
    if ((pFormat->pitch[0] > UTILS_DMA_MAX_PITCH)
        ||
        (cbcrPitch > UTILS_DMA_MAX_PITCH))
    {
        doLineXfer = TRUE;
    }
    ...
    for (frameId = 0; frameId < numFrames; frameId++)
    {
        ...
        else
        {
            ...
            for (lineIdx = 0; lineIdx < pFormat->height; lineIdx++)
            {
                numTx=0;

                blankFrameInfo[numTx].destAddr[0] = bufBaseAddrY + (lineIdx * pFormat->pitch[0]);
                blankFrameInfo[numTx].destPitch[0] = pFormat->pitch[0];
                blankFrameInfo[numTx].destAddr[1] = bufBaseAddrCbCr + (lineIdx * cbcrPitch);
                blankFrameInfo[numTx].destPitch[1] = cbcrPitch;

                blankFrameInfo[numTx].dataFormat  = pFormat->dataFormat;
                blankFrameInfo[numTx].width = pFormat->width;
                blankFrameInfo[numTx].height = 1;
                blankFrameInfo[numTx].fillColorYUYV =
                                     UTILS_DMA_GENERATE_FILL_PATTERN(
                                                        0x80,
                                                        0x80,
...
Int32 SwMsLink_drvCheckBlankOutputBuffer(SwMsLink_Obj * pObj, FVID2_Frame *pFrame)
{
        ...
        (frameInfo->swMsBlankOutBuf))
    {
	    ...
	    blankFrameInfo[numTx].fillColorYUYV =
            UTILS_DMA_GENERATE_FILL_PATTERN(0x10 ,
                                            0x80,
                                            0x80);
        blankFrameInfo[numTx].startX = 0;
        blankFrameInfo[numTx].startY = 0;
...
static Int32 SwMsLink_drvCreateDupObj(SwMsLink_Obj * pObj)
{
    ...
    pObj->enableDupOutputFrame = SWMS_LINK_ENABLE_DUP_OUPUT_FRAME;
                ...
                UInt32 curCoeffId, Bool isDei)
{
    ...
    coeffPrms.hScalingSet = curCoeffId;
    coeffPrms.vScalingSet = curCoeffId;
...
Int32 SwMsLink_drvSelectScCoeffs (SwMsLink_Obj * pObj)
{
    ...
    UInt32 inH, outH, coeffId, numerator, denominator;
    Bool isDei;
    Int32 instId;
    ...
    if (pObj->DrvObj[instId].fvidHandle)
        SwMsLink_drvSetScCoeffs(pObj, pObj->DrvObj[instId].fvidHandle, coeffId, isDei);
...
Int32 SwMsLink_drvCreateDeiDrv(SwMsLink_Obj * pObj, SwMsLink_DrvObj *pDrvObj)
{
    ...
    {
        SwMsLink_drvSetScCoeffs(pObj, pDrvObj->fvidHandle, VPS_SC_US_SET, TRUE);
    }
...
Int32 SwMsLink_drvCreate(SwMsLink_Obj * pObj, SwMsLink_CreateParams * pPrm)
{
    ...
    SwMsLink_drvCreateChannelObj(pObj);	//HOW TO DO
...
Int32 SwMsLink_drvProcessData(SwMsLink_Obj * pObj)
{
    ...
    {
        SwMsLink_drvLock(pObj);
        SwMsLink_drvQueueInputFrames(pObj,&frameList);	//HOW TO DO
        SwMsLink_drvUnlock(pObj);
...
Int32 SwMsLink_drvMakeFrameLists(SwMsLink_Obj * pObj, FVID2_Frame * pOutFrame)
{
    ...
    SwMsLink_drvUpdateChCurrentInFrame(pObj);	//HOW TO DO
...
Int32 SwMsLink_drvDoScaling(SwMsLink_Obj * pObj)
{
    ...
    if ((pObj->layoutParams.outputFPS <= 30) && (pObj->enableDupOutputFrame))
    {
        if (pOutFrame)
        {
	...
	#if 0
	System_FrameInfo	*pFrameInfo = (System_FrameInfo *) pOutFrame->appData;
	if (pFrameInfo != NULL)
	{
		Vps_printf("w[%d] h[%d] p[%d] p[%d] dataFormat[%d]\n",\
		...
		pFrameInfo->rtChInfo.pitch[0] = 3840;
		pFrameInfo->rtChInfo.pitch[1] = 3840;
		pFrameInfo->rtChInfo.width = 1280;
		pFrameInfo->rtChInfo.height = 720;
		pFrameInfo->rtChInfo.dataFormat = FVID2_DF_YUV422I_YUYV;
	#endif
   ...
   // Vps_printf("SwMsLink_drvDoScaling: SYSTEM_CMD_NEW_DATA!\n");
                                 ...
                                 SwMsLink_LayoutPrm * layoutParams)
{
    ...
    {
        pObj->timerPeriod =
              (1000/(layoutParams->outputFPS+(layoutParams->outputFPS/50)));
