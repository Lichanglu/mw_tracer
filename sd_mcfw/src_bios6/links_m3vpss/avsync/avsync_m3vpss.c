...
Int32 avsync_validate_synchcfg_params(AvsyncLink_LinkSynchConfigParams *cfg)
{
    ...
    UTILS_assertError(((cfg->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                      &&
                           ...
                           AvsyncLink_LinkSynchConfigParams *cfg)
{
    ...
    if (status == AVSYNC_S_OK)
    {
         UTILS_assertError(((pObj->srObj->numAudQueCreated == 0)
                            &&
                           (pObj->srObj->numVidQueCreated[cfg->displayID] == 0)),
                           status,
                           AVSYNC_E_INVALIDPARAMS,
                           SYSTEM_LINK_ID_AVSYNC,
                           -1);
    ...
    {
        AVSYNC_CRITICAL_BEGIN();
        pObj->displayId2vidSyncLinkMap[cfg->displayID] =
            cfg->videoSynchLinkID;
        pObj->displayId2audDevMap[cfg->displayID] =
            cfg->audioDevId;
        pObj->syncMasterChNum[cfg->displayID] =
            cfg->syncMasterChnum;
        for (i = 0; i < cfg->numCh; i++)
        {
            vidQueCfg = &cfg->queCfg[i];
            pObj->srObj->queCfg[cfg->displayID][vidQueCfg->chNum] =
                *vidQueCfg;
        }
        AVSYNC_CRITICAL_END();
    }
...
Void avsync_init_cfgparams(AvsyncLink_Obj *pObj)
{
    ...
    for (devId = 0;devId < UTILS_ARRAYSIZE(pObj->srObj->queCfg);devId++)
    {
        ...
        #ifdef TI_814X_BUILD
        /* !HACK HACK. This is needed to set correct SwMs instanceid
        * for SD dislay.
        */
        if (devId == 2)
        {
            synchCfg.videoSynchLinkID = SYSTEM_LINK_ID_SW_MS_MULTI_INST_1;
        }
        #endif
        synchCfg.numCh = 0;
...
//	Vps_printf("queueMem = %p, maxElements = %d, chNum = %d, displayID = %d, syncLinkID = %d\n",
//					cp->queueMem, cp->maxElements, cp->chNum, cp->displayID, cp->syncLinkID);

//	Vps_printf("avsync_map_linkid2displayid(pObj,cp->syncLinkID) = %d, AVSYNC_INVALID_DISPLAY_ID = %d\n",
//					avsync_map_linkid2displayid(pObj,cp->syncLinkID), AVSYNC_INVALID_DISPLAY_ID);
    ...
    UTILS_assertError(((cp->queueMem != NULL)
                       ...
                       &&
                       ((avsync_map_linkid2displayid(pObj,cp->syncLinkID) !=
    ...
    while (!done)
    {
        ...
        switch (cmd)
        {
            case AVSYNC_LINK_CMD_PAUSE:
            {
                ...
                if (pauseParams->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(pauseParams->displayID);
                    if (AVSYNC_ALL_CHANNEL_ID == pauseParams->chNum)
                    {
                        avsync_pause_all_channels(pObj,
                                                  pauseParams->displayID);
                        status = AVSYNC_S_OK;
                    }
                    else
                    {
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(pauseParams->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
            ...
            case AVSYNC_LINK_CMD_UNPAUSE:
            {
                ...
                Avsync_PlayerTimeObj *playerTime;
                if (unpauseParams->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(unpauseParams->displayID);

                    if (AVSYNC_ALL_CHANNEL_ID == unpauseParams->chNum)
                    {
                        avsync_unpause_all_channels(pObj,
                                                    unpauseParams->displayID);
                        status = AVSYNC_S_OK;
                    }
                    else
                    {
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(unpauseParams->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
            ...
            case AVSYNC_LINK_CMD_TIMESCALE:
            {
                ...
                if (timeScaleParams->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(timeScaleParams->displayID);

                    if (AVSYNC_ALL_CHANNEL_ID == timeScaleParams->chNum)
                    {
                        avsync_timescale_all_channels(pObj,
                                                      timeScaleParams->displayID,
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(timeScaleParams->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
            ...
            case AVSYNC_LINK_CMD_STEP_FWD:
            {
                ...
                if (stepFwdParams->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(stepFwdParams->displayID);

                    if (AVSYNC_ALL_CHANNEL_ID == stepFwdParams->chNum)
                    {
                        avsync_stepfwd_all_channels(pObj,
                                                    stepFwdParams->displayID);
                        status = AVSYNC_S_OK;
                    }
                    else
                    {
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(stepFwdParams->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
            ...
            case AVSYNC_LINK_CMD_SET_FIRST_AUDPTS:
            {
                ...
                if (audPTSParams->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(audPTSParams->displayID);

                    playerTime =
                        avsync_get_player_timeobj(pObj,
                                                  audPTSParams->displayID,
                                                  audPTSParams->chNum);
                    synchCfg =
                        avsync_get_avsync_config(pObj,
                                                 audPTSParams->displayID,
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(audPTSParams->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
            ...
            case AVSYNC_LINK_CMD_SET_FIRST_VIDPTS:
            {
                ...
                if (vidPTSParams->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(vidPTSParams->displayID);

                    playerTime =
                        avsync_get_player_timeobj(pObj,
                                                  vidPTSParams->displayID,
                                                  vidPTSParams->chNum);
                    synchCfg =
                        avsync_get_avsync_config(pObj,
                                                 vidPTSParams->displayID,
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(vidPTSParams->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
            ...
            case AVSYNC_LINK_CMD_RESET_PLAYERTIME:
            {
                ...
                if (resetPlayerParams->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(resetPlayerParams->displayID);

                    if (AVSYNC_ALL_CHANNEL_ID == resetPlayerParams->chNum)
                    {
                        avsync_reset_all_channels(pObj,
                                                  resetPlayerParams->displayID);
                        status = AVSYNC_S_OK;
                    }
                    else
                    {
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(resetPlayerParams->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
            ...
            case AVSYNC_LINK_CMD_PLAY:
            {
                ...
                if (playParams->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(playParams->displayID);

                    if (AVSYNC_ALL_CHANNEL_ID == playParams->chNum)
                    {
                        avsync_play_all_channels(pObj,
                                                 playParams->displayID,
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(playParams->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
            ...
            case AVSYNC_LINK_CMD_SCAN:
            {
                ...
                if (scanParams->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(scanParams->displayID);

                    if (AVSYNC_ALL_CHANNEL_ID == scanParams->chNum)
                    {
                        avsync_do_scan_all_channels(pObj,
                                                    scanParams->displayID,
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(scanParams->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
            ...
            case AVSYNC_LINK_CMD_SET_VIDEO_BACKEND_DELAY:
            {
                ...
                if (backendDelay->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(backendDelay->displayID);

                    if (backendDelay->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                    {
                        avsync_set_video_backend_delay(
                          &pObj->displayObj[backendDelay->displayID],
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(backendDelay->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
            ...
            case AVSYNC_LINK_CMD_SEEK:
            {
                ...
                if (seekParams->displayID < AVSYNC_MAX_NUM_DISPLAYS)
                {
                    AVSYNC_CFG_MUTEX_ACQUIRE(seekParams->displayID);

                    if (AVSYNC_ALL_CHANNEL_ID == seekParams->chNum)
                    {
                        avsync_do_player_seek_all_channels(pObj,
                                                    seekParams->displayID,
                    ...
                    AVSYNC_CFG_MUTEX_RELEASE(seekParams->displayID);
                }
                else
                {
                    status = AVSYNC_E_INVALIDPARAMS;
