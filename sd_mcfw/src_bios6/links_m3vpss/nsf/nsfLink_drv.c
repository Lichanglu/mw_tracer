...
Int32 NsfLink_drvPrintStatistics(NsfLink_Obj * pObj, Bool resetAfterPrint)
{
    ...
    Vps_printf( " \n"
            ...
            " \n"
            " \n"
            " CH  | In Recv In Reject In Process Out User Out \n"
            " Num | FPS     FPS       FPS        FPS Skip FPS \n"
            " ------------------------------------------------\n",
            pObj->name,
            elaspedTime,
    ...
    for (chId = 0; chId < pObj->inQueInfo.numCh; chId++)
    {
		...
		Utils_bufPrintStatus("nsf in", &pObj->linkChInfo[chId].bufInQue);

    }

		Utils_bufPrintStatus("nsf out",&pObj->bufOutQue[0]);
    ...
    if (inFrameList[0].numFrames)
    {
        ...
        for (queueId = 0; queueId < NSF_LINK_MAX_OUT_QUE; queueId++)
        {
            if (sendMsgToTsk & 0x1)
            {
				...
				//Vps_printf("NsfLink_drvDoNsfFilter: SYSTEM_CMD_NEW_DATA!\n");
...
Int32 NsfLink_drvInitCh(NsfLink_Obj * pObj)
{
    ...
    for (chId = 0; chId < pObj->nsfCreateParams.numCh; chId++)
    {
        ...
        if (NSF_LINK_NUM_BUFS_PER_CH_DEFAULT == pObj->createArgs.numBufsPerCh || pObj->createArgs.numBufsPerCh > NSF_LINK_FRAMES_PER_CH_MAX)
        {
