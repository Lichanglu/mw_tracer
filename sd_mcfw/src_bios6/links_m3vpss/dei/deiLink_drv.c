    ...
    for (outId = 0u; outId < DEI_LINK_MAX_OUT_QUE; outId++)
    {
        if (pObj->createArgs.enableOut[outId])
        {
            ...
            for (chId = 0u; chId < pObj->inQueInfo.numCh; chId++)
            {
				...
				Vps_printf("dei: pInChInfo->height = %d\n", pInChInfo->height);
				Vps_printf("dei: pInChInfo->scanFormat = %d\n", pInChInfo->scanFormat);
				...
				Vps_printf("dei: pFormat->width = %d\n", pFormat->width);
				Vps_printf("dei: pFormat->height = %d\n", pFormat->height);
                switch (outId)
                {
...
Int32 DeiLink_drvCreateChObj(DeiLink_Obj * pObj, UInt32 chId)
{
    ...
    for (outId = 0u; outId < DEI_LINK_MAX_OUT_QUE; outId++)
    {
        ...
        if (TRUE == pObj->createArgs.enableOut[outId])
        {
			...
			Vps_printf("--------------------------------------------------\n");
			Vps_printf("pInChInfo->width = %d\n", pInChInfo->width);
			Vps_printf("pInChInfo->height = %d\n", pInChInfo->height);
			Vps_printf("pOutChInfo->width = %d\n", pOutChInfo->width);
			Vps_printf("pOutChInfo->height = %d\n", pOutChInfo->height);
			Vps_printf("--------------------------------------------------\n");
...
Int32 DeiLink_drvCreate(DeiLink_Obj * pObj, DeiLink_CreateParams * pPrm)
{
	...
	Vps_printf("dei: height = %d\n", pObj->inQueInfo.chInfo[0].height);
	Vps_printf("dei: scanFormat = %d\n", pObj->inQueInfo.chInfo[0].scanFormat);
	Vps_printf("dei: dataFormat = %d\n", pObj->inQueInfo.chInfo[0].dataFormat);
	Vps_printf("dei: pitch[0] = %d\n", pObj->inQueInfo.chInfo[0].pitch[0]);
	Vps_printf("dei: pitch[1] = %d\n", pObj->inQueInfo.chInfo[0].pitch[1]);
	Vps_printf("dei: pitch[2] = %d\n", pObj->inQueInfo.chInfo[0].pitch[2]);
	pObj->inQueInfo.chInfo[0].width= 1920;
	pObj->inQueInfo.chInfo[0].height = 600;
...
Int32 DeiLink_drvQueueFramesToChQue(DeiLink_Obj * pObj)
{
	...
	//	Vps_printf("pChObj->nextFid = %d\n",pChObj->nextFid);
			...
			if(pInFrameInfo->rtChInfo.scanFormat == FVID2_SF_PROGRESSIVE)
				pChObj->nextFid = 0;

			if(pInFrameInfo->rtChInfoUpdate == 1){

				//Vps_printf("pInFrameInfo->rtChInfoUpdate w[%d] h[%d] p[%d][%d] ch[%d] [%d]",pInFrameInfo->rtChInfo.width,\
					//pInFrameInfo->rtChInfo.height,pInFrameInfo->rtChInfo.pitch[0],pInFrameInfo->rtChInfo.pitch[1],\
				//	pFrame->channelNum,pInFrameInfo->rtChInfo.scanFormat);
	...
	//			Vps_printf("nextFid = %d, fid = %d\n", pChObj->nextFid, pFrame->fid);
			}

            if (pChObj->nextFid == pFrame->fid)//HOW TO DO ????
            {
                ...
                if( doFrameDrop == TRUE)
                {
      //          	Vps_printf("dei: doFrameDrop == TRUE\n");
...
Int32 DeiLink_drvChangeChFidFlag(DeiLink_Obj * pObj, UInt32 chid)
{
    Int32 status = FVID2_SOK;
    DeiLink_ChObj *pChObj;
	...
	Vps_printf("change fid flag : chid = %d\n", chid);
