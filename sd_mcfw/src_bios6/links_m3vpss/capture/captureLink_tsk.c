...
#if 0
UInt8 gCapAdjustInputFrameStack[CAPTURE_LINK_TSK_STACK_SIZE];
Utils_TskHndl gCapAdjustInputFrameTsk;
Void capture_inputframe_adjust_tsk(struct Utils_TskHndl * pTsk, Utils_MsgHndl * pMsg)
{
	UInt32 index = 0;
	UInt32 count = 0;
                         ...
                         Utils_MsgHndl ** pMsg, Bool * done, Bool * ackMsg)
{
    ...
    *pMsg = NULL;
    ...
    while (!runDone)
    {
        ...
        switch (cmd)
        {
			case CAPTURE_LINK_CMD_CHECK_STATUS:
				Utils_tskAckOrFreeMsg(pRunMsg, status);

				//Vps_printf("CAPTURE_LINK_CMD_CHECK_STATUS [count_check = %d]\n", count_check);
				break;
            ...
            case CAPTURE_LINK_CMD_SKIP_ALTERNATE_FRAMES:

                status = CaptureLink_drvSkipAlternateFrames(
                            pObj,
                            (CaptureLink_SkipAlternateFrames *)Utils_msgGetPrm(pRunMsg)
                            );
                Utils_tskAckOrFreeMsg(pRunMsg, status);
                break;
			...
			case CAPTURE_LINK_CMD_A8_DETECT_VIDEO:
				{
					...
					Hwi_restore(key);
					Vps_printf(" pVidStatus->frameWidth:[%d]  pVidStatus->frameHeight:[%d]\n",
								pVidStatus->frameWidth,  pVidStatus->frameHeight);
	                /* ACK or free message before proceding */
	                Utils_tskAckOrFreeMsg(pRunMsg, status);
				}
			...
			case CAPTURE_LINK_CMD_SET_OUTPUT_FRAMERATE:
                status = CaptureLink_drvSet_Minni_FrameRate(
                            pObj,
                            (CaptureLink_SetChannelFramerate *)Utils_msgGetPrm(pRunMsg)
                            );
                Utils_tskAckOrFreeMsg(pRunMsg, status);
                break;
#endif
    ...
    while (!done)
    {
        ...
        switch (cmd)
        {
            ...
            case CAPTURE_LINK_CMD_SKIP_ALTERNATE_FRAMES:
                ...
                status = CaptureLink_drvSkipAlternateFrames(
                            pObj,
                            (CaptureLink_SkipAlternateFrames *)Utils_msgGetPrm(pMsg)
                            );
                Utils_tskAckOrFreeMsg(pMsg, status);
                break;
...
Int32 CaptureLink_init()
{
    ...
    */
	...
	char ad_name[32] = {0};
	memset(ad_name, 0, 32);
	UTILS_SNPRINTF(ad_name, "cap_adjust_infps");
	status= Reach_tskCreate(&gCapAdjustInputFrameTsk,
						 capture_inputframe_adjust_tsk,
						 CAPTURE_LINK_TSK_PRI,
						 gCapAdjustInputFrameStack,
						 CAPTURE_LINK_TSK_STACK_SIZE, pObj, ad_name);
	UTILS_assert(status == FVID2_SOK);
#endif
