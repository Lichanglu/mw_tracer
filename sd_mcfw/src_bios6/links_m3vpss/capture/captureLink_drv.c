...
Int32 CaptureLink_drvChObjReset(CaptureLink_Obj * pObj)
{
    ...
    for(queId=0; queId<CAPTURE_LINK_MAX_OUT_QUE; queId++)
    {
        for(chId=0; chId<CAPTURE_LINK_MAX_CH_PER_OUT_QUE; chId++)
        {
            ...
            pChObj->skipAlternateFields = FALSE;
            pChObj->skipAlternateFieldsThreshold = 0;
...
Int32 CaptureLink_drvSkipAlternateFrames(
            CaptureLink_Obj * pObj,
            CaptureLink_SkipAlternateFrames *pPrm
            )
{
    CaptureLink_ChObj *pChObj;
    CaptureLink_InstObj *pInst;
    ...
    for(chId=0; chId<numChInQue; chId++)
    {
        ...
        if(pPrm->skipAlternateFramesChBitMask & (1<<chId))
        {
            pChObj->skipAlternateFields = TRUE;
        }

        if(pChObj->skipAlternateFields==FALSE)
        {
...
#if 0
/* 设置capture给出的最低帧率，以使得后面的link不需要处理或忽略过多的帧 */
Int32 CaptureLink_drvSet_Minni_FrameRate(
            CaptureLink_Obj * pObj,
            CaptureLink_SetChannelFramerate *pPrm
            )
{
	/* 依赖于 pChObj->inputFrameRate 值 */
    CaptureLink_ChObj *pChObj;
    CaptureLink_InstObj *pInst;
    Vps_CaptFrameSkip frameSkipMask;
	...
	if(pPrm->chId >= CAPTURE_LINK_MAX_OUTPUT_PER_INST)
        return FVID2_EFAIL;
	...
	if(pChObj->inputFrameRate <= 0)
		return FVID2_EFAIL;

	frameSkipMask.channelNum = CaptureLink_makeChannelNum(pPrm->queId, pPrm->chId);
    frameSkipMask.frameSkipMask = 0x0;

	pPrm->framerate/pChObj->inputFrameRate

	if((pPrm->framerate * 2) <= pChObj->inputFrameRate){
		frameSkipMask.frameSkipMask = 0x2AAAAAAA;		// 取 1/2
	}
	else if((pPrm->framerate * 3) <= (pChObj->inputFrameRate * 2)){
		frameSkipMask.frameSkipMask = 0x36DB6DB6;		// 取 2/3
	}
	else {
		frameSkipMask.frameSkipMask = 0x0;				// 取 1/1
	}
...
Int32 CaptureLink_drvCheckAndSetFrameSkipMask(CaptureLink_Obj * pObj, FVID2_Frame *pFrame)
{
    ...
    if(pFrame->fid!=0)
    {
        pChObj->skipAlternateFieldsThreshold++;

        if(pChObj->skipAlternateFieldsThreshold > CAPTURE_LINK_SKIP_ALTERNATE_FIELDS_THRESHOLD)
        {
            pChObj->skipAlternateFieldsThreshold = 0;
...
Int32 CaptureLink_drvCreateInst(CaptureLink_Obj * pObj, UInt16 instId)
{
    ...
    else
    {
        ...
        {
        	if((pInstPrm->standard == FVID2_STD_NTSC)||(pInstPrm->standard == FVID2_STD_PAL)){
			  pVipCreateArgs->videoCaptureMode =
			  	VPS_CAPT_VIDEO_CAPTURE_MODE_SINGLE_CH_NON_MUX_EMBEDDED_SYNC;
			  pVipCreateArgs->videoIfMode = VPS_CAPT_VIDEO_IF_MODE_8BIT;
		}else{
                            ...
                            CaptureLink_CreateParams * pPrm)
{
    ...
    for (instId = 0; instId < pPrm->numVipInst; instId++)
    {
        ...
        for (i = 0; i < pObj->instObj[i].createArgs.numCh;i++)
        {
    ...
    for (streamId = 0; streamId < CAPTURE_LINK_MAX_OUTPUT_PER_INST; streamId++)
    {
        ...
        if (frameList.numFrames)
        {
            for (frameId = 0; frameId < frameList.numFrames; frameId++)
            {
				...
				if((Utils_getCurTimeInMsec() - lasttime)>= 2000){
					lasttime = Utils_getCurTimeInMsec();
				}
				...
				if(pVidStatus->isInterlaced == FVID2_SF_PROGRESSIVE){
					if(pFrame->fid == 1)
						pFrame->fid = 0;
					else if(pFrame->fid ==0)
						pFrame->fid =1;
					else
						Vps_printf("pFrame->fid =%d\n",pFrame->fid);


					//Vps_printf("[%d * %d], pFrame->fid =%d\n", pVidStatus->frameWidth, pVidStatus->frameHeight, pFrame->fid);

					//PAL just for CL3000
					if (pVidStatus->frameWidth == 720 && pVidStatus->frameHeight == 576/2)
					{
						if(pFrame->fid == 1)
							pFrame->fid = 0;
						else if(pFrame->fid ==0)
				...
				#endif
		//Vps_printf("FVID2_SF_PROGRESSIVE=%d,pVidStatus->isInterlaced=%d,pFrame->fid=%d\n",
		//FVID2_SF_PROGRESSIVE,pVidStatus->isInterlaced ,pFrame->fid);
			//	if(pVidStatus->isInterlaced == FVID2_SF_PROGRESSIVE){
			//		pFrame->fid = 0;
			//	}
				...
				if(pVidStatus->isInterlaced == 0){
					...
					pFrameInfo->rtChInfo.width	  = pVidStatus->frameWidth;
					pFrameInfo->rtChInfo.height   = pVidStatus->frameHeight;
				}
...
Int32 CaptureLink_drvDetectVideoStandard(CaptureLink_Obj * pObj)
{
    ...
    Int32 status, instId, retryCount;
