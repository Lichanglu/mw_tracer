...
Int32 SelectLink_drvSetOutQueChInfo2(SelectLink_Obj * pObj, SelectLink_OutQueChInfo *pPrm)
{
    UInt32 inChNum, outChNum;
    SelectLink_ChInfo *pChInfo;
	SelectLink_OutQueChInfo *pPrevPrm;
    ...
    for(outChNum=0; outChNum<pPrevPrm->numOutCh; outChNum++)
    {
		...
		if((SYSTEM_GET_LINK_ID(pObj->tskId)- SYSTEM_LINK_ID_SELECT_0) == 2)
		{
			Vps_printf("remove:pPrevPrm->numOutCh = %d inChNum = %d outChNum = %d\n",pPrevPrm->numOutCh,inChNum,outChNum);
		}
    }
    ...
    for(outChNum=0; outChNum<pPrm->numOutCh; outChNum++)
    {
		...
		if((SYSTEM_GET_LINK_ID(pObj->tskId)- SYSTEM_LINK_ID_SELECT_0) == 2)
		{
			Vps_printf("mapped:pPrevPrm->numOutCh = %d inChNum = %d  outChNum = %d queId = %d\n",pPrm->numOutCh,inChNum,outChNum,pChInfo->queId);
		}
#endif
    ...
    if (frameList.numFrames)
    {
        for (frameId = 0; frameId < frameList.numFrames; frameId++)
        {
            ...
            else
            {
                ...
                pFrameInfo->selectOrgChannelNum[SelectID] = pFrame->channelNum;
                               ...
                               FVID2_FrameList * pFrameList)
{
    ...
    for (frameId = 0; frameId < pFrameList->numFrames; frameId++)
    {
		...
		SelectID = SYSTEM_GET_LINK_ID(pObj->tskId)- SYSTEM_LINK_ID_SELECT_0;
		//Vps_printf("Select ret---->Ret[channelNum] = %d %d\n",pFrame->channelNum,pFrameInfo->selectOrgChannelNum);
        pFrame->channelNum = pFrameInfo->selectOrgChannelNum[SelectID];
    }
