    ...
    for (inQue = 0; inQue < pCreateArgs->numInQue; inQue++)
    {
        ...
        if (pObj->inFrameList.numFrames)
        {
			...
			//	pFrameInfo->mergeChannelNum[MergeID] = pFrame->channelNum;
				...
				#if 0
				if((pFrame->channelNum == 2) && (MergeID == 2) )
				{

					Vps_printf("Merger in----> IN[channelNum] = %d OUT[channelNum] = %d\n",pFrameInfo->mergeChannelNum[MergeID],pFrame->channelNum);
				}
				#endif
                               ...
                               FVID2_FrameList * pFrameList)
{
    ...
    for (frameId = 0; frameId < pFrameList->numFrames; frameId++)
    {
        ...
        UTILS_assert(pFrameInfo != NULL);
	 	...
	 	//Vps_printf("Merger ret---->RET[channelNum] = %d In[channelNum]= %d IN Que = %d chn = %d\n",pFrame->channelNum, pFrameInfo->mergeChannelNum,pObj->outQueChToInQueMap[pFrame->channelNum],pObj->outQueChMap[pFrame->channelNum]);
		/* 获取队列ID */
    ...
    for (inQue = 0; inQue < pCreateArgs->numInQue; inQue++)
    {
		//Vps_printf("Merger retp---->prdID = %d id =%d num =%d\n",pCreateArgs->inQueParams[inQue].prevLinkId,pCreateArgs->inQueParams[inQue].prevLinkQueId,pObj->freeFrameList[inQue].numFrames);
