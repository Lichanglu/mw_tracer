 ...
 */
#if(defined(TI_814X_BUILD) || defined (TI_8107_BUILD) || defined(TI816X_256M_DDR))
...
#define VPSS_ALG_LINK_TSK_PRI            (8)
...
#define VPSS_ALG_LINK_TSK_PRI            (8)
/* Run-time priority of the Alg link when OSD process frames is called,
   priority is reverted to VPSS_ALG_LINK_TSK_PRI after OSD completes
*/
#define VPSS_ALG_LINK_TSK_PRI_OSD        (1)
...
typedef struct {
    ...
    Void *pOrgListMPElem;
    UInt32 mergeChannelNum[4];
	UInt32 sclrOrgChannelNum;
