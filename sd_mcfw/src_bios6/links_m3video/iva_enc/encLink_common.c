...
EncDec_ResolutionClass enclink_get_resolution_class(UInt32 width, UInt32 height)
{
	...
	Vps_printf("__ width = %d, __ height = %d\n", width, height);

	UTILS_assert((width <= UTILS_ENCDEC_RESOLUTION_CLASS_16MP_WIDTH)
	             && (height <= UTILS_ENCDEC_RESOLUTION_CLASS_16MP_HEIGHT));

	/*增加4K分辨率 */
	if ((width > UTILS_ENCDEC_RESOLUTION_CLASS_4K_WIDTH) ||
	        (height > UTILS_ENCDEC_RESOLUTION_CLASS_4K_HEIGHT))
	{
		resClass = UTILS_ENCDEC_RESOLUTION_CLASS_16MP;
	}
	else
		if((width > UTILS_ENCDEC_RESOLUTION_CLASS_3840x1200_WIDTH) ||
		        (height > UTILS_ENCDEC_RESOLUTION_CLASS_3840x1200_HEIGHT))
		{
			resClass = UTILS_ENCDEC_RESOLUTION_CLASS_4K;
		}
		else
                                      ...
                                      UInt32 * pWidth, UInt32 * pHeight)
{
	...
	switch (resClass)
	{
		case UTILS_ENCDEC_RESOLUTION_CLASS_4K:
			*pWidth  = UTILS_ENCDEC_RESOLUTION_CLASS_4K_WIDTH;
			*pHeight = UTILS_ENCDEC_RESOLUTION_CLASS_4K_HEIGHT;
			break;
		case UTILS_ENCDEC_RESOLUTION_CLASS_3840x1200:
			*pWidth  = UTILS_ENCDEC_RESOLUTION_CLASS_3840x1200_WIDTH;
			*pHeight = UTILS_ENCDEC_RESOLUTION_CLASS_3840x1200_HEIGHT;
			break;
...
static Int32 EncLink_codecCreateOutObj(EncLink_Obj * pObj)
{
	...
	for (i = 0; i < pOutObj->numAllocPools; i++)
	{
		Vps_printf("EncLink_codecCreateOutObj: numAllocPools[%d]  buf_size[%d] outNumBufs[%d]\n",\
		           pOutObj->numAllocPools,pOutObj->buf_size[i],pOutObj->outNumBufs[i]);
		...
		Vps_printf("\n\n\n");
		Vps_printf("________________ bitbuffer phyAddr = %x\n", pOutObj->outBufs[totalBufCnt].phyAddr);
		Vps_printf("________________ bitbuffer buffSize = %x\n", pOutObj->outBufs[totalBufCnt].bufSize);
		Vps_printf("\n\n\n");
        ...
        UInt32 chId)
{
	...
	pChAlgCreatePrm->level = IH264_LEVEL_41;
...
static Int32 EncLink_codecCreateEncObj(EncLink_Obj * pObj, UInt32 chId)
{
	...
	Vps_printf("retval =%d,pChObj->algObj.algCreateParams.format=%d,IVIDEO_MJPEG=%d,IVIDEO_H264HP=%d,\n",retVal,pChObj->algObj.algCreateParams.format,IVIDEO_MJPEG,IVIDEO_H264HP);
...
Int32 EncLink_codecCreate(EncLink_Obj * pObj, EncLink_CreateParams * pPrm)
{
...
#ifdef SYSTEM_DEBUG_ENC
		Vps_printf(" %d: ENCODE: Creating CH%d of %d x %d, pitch = (%d, %d) [%s] [%s], bitrate = %d Kbps [%d]... \n",
		           Utils_getCurTimeInMsec(),
		           chId,
	...
	for (tskId = 0; tskId < NUM_HDVICP_RESOURCES; tskId++)
	{
		status = Utils_queCreate(&pObj->encProcessTsk[tskId].processQue,
		                         (ENC_LINK_MAX_OUT_FRAMES/NUM_HDVICP_RESOURCES),
...
static Int32 EncLink_codecQueueFramesToChQue(EncLink_Obj * pObj)
{
	...
	if (frameList.numFrames)
	{
		...
		//Vps_printf("EncLink_codecQueueFramesToChQue: System_getLinksFullFrames!!!\n");
        ...
        for (frameId = 0; frameId < frameList.numFrames; frameId++)
        {
            ...
            {
#if 0
            	if( 30 == pChObj->algObj.algDynamicParams.targetFrameRate ){
			 skipFrame = EncLink_doSkipFrame_Only30fps(pChObj, pFrame->channelNum);
            	}else{
			//Vps_printf(" pChObj->algObj.algDynamicParams.targetFrameRate/1000=%d", pChObj->algObj.algDynamicParams.targetFrameRate/1000);
...
static Int32 EncLink_codecSubmitData(EncLink_Obj * pObj)
{
	...
	for (chId = 0; chId < pObj->inQueInfo.numCh; chId++)
	{
		...
		while(numReqObjPerProcess < pChObj->numReqObjPerProcess)
		{
			...
			pObj->reqQueCount++;
			UTILS_assert(ENC_LINK_MAX_REQ >= pObj->reqQueCount);
...
static Int32 EncLink_codecGetProcessedData(EncLink_Obj * pObj)
{
	...
	if (outBitBufList.numBufs)
	{
		...
		//	Vps_printf("EncLink_codecGetProcessedData: SYSTEM_CMD_NEW_DATA!!\n");
...
Int32 EncLink_codecSetFps(EncLink_Obj * pObj, EncLink_ChFpsParams * params)
{
	...
	inputFps = pObj->ulHistoryFrameRate[params->chId];
	if(inputFps > 85){
		inputFps = 85;
	}
	EncLink_ChInputFpsParam params1;
	params1.chId     = params->chId;
	params1.inputFps = inputFps;
	EncLink_codecInputSetFps(pObj,&params1);
...
#if 0
#define 	INTERVAL_TIME      1000
#define MAX_INTERVAL_TIME	690000
#define	MAX_MP_TIME		720000
#define 	MP_INTERVAL_TIME      3000

static Bool EncLink_doSkipFrame_interval(EncLink_ChObj *pChObj, Int32 chId)
{
	UInt32 currTime =  Utils_getCurTimeInMsec();
	UInt32 interval = 40;
...
Bool  EncLink_doSkipFrame(EncLink_ChObj *pChObj, Int32 chId)
{
	//VGA
	if(4 == chId ){
		if( TRUE == EncLink_doSkipFrame_interval(pChObj,chId)	){
			return TRUE;
		}
		return FALSE;
	}
...
{
	static	UInt32		prevTime[SYSTEM_MAX_CH_PER_OUT_QUE] = {0};
	static	UInt32		now_frame[SYSTEM_MAX_CH_PER_OUT_QUE] = {0};
	UInt32				ctrl_frame = 0;
	UInt32				detaTime;
	UInt32				newTime;
	UInt32				everyTime = 0;

	ctrl_frame = (pChObj->algObj.algDynamicParams.targetFrameRate);
	if(ctrl_frame > 60000)
		ctrl_frame = 60000;
	...
	everyTime = (1000000)/(ctrl_frame+1);
	newTime = Utils_getCurTimeInMsec()*1000;

	if(prevTime[chId] == 0)
	{
		prevTime[chId] = newTime;
		now_frame[chId] = 0;
		return FALSE;
	}

	detaTime = (newTime - prevTime[chId]);
	if(chId == 0)
		Vps_printf("dataTime = %d, everyTime = %d\n", detaTime/1000, everyTime/1000);
	if(detaTime >= everyTime)
	{
		prevTime[chId] = newTime ;
		if(chId == 0)
			Vps_printf("dataTime = %d\n", detaTime/1000);
		return FALSE;
	}
	else
	{
        ...
        EncLink_ReqObj * reqObj, UInt32 chId)
{
	...
	{
		pFrameInfo->rtChInfo.height   = Reach_floor(pFrameInfo->rtChInfo.height,   8);
		pFrameInfo->rtChInfo.width    = Reach_floor(pFrameInfo->rtChInfo.width,   16);
		...
		{
			//Vps_printf("pFrameInfo->rtChInfo.width = %d  ,pFrameInfo->rtChInfo.height = %d  chDynamicParams->inputPitch =%d %d\n",\
			          // pFrameInfo->rtChInfo.width,pFrameInfo->rtChInfo.height, chDynamicParams->inputPitch,pFrameInfo->rtChInfo.pitch[0]);
		...
		{
			//Vps_printf("pFrameInfo->rtChInfo.width = %d  ,pFrameInfo->rtChInfo.height = %d\n",pFrameInfo->rtChInfo.width,pFrameInfo->rtChInfo.height);
			switch (pChObj->algObj.algCreateParams.format)
			{
				case IVIDEO_H264BP:
				case IVIDEO_H264MP:
                                   ...
                                   EncLink_ReqBatch *pReqObjBatch)
{
	        ...
	        (UTILS_ENCDEC_RESOLUTION_CLASS_720P_WIDTH < inputFrameWidth))
	{
		        ...
		        (UTILS_ENCDEC_RESOLUTION_CLASS_D1_WIDTH < inputFrameWidth))
		{
			...
			if (UTILS_ENCDEC_RESOLUTION_CLASS_D1_WIDTH >= inputFrameWidth)
			{
