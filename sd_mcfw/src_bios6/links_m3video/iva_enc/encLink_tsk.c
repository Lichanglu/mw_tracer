...
#pragma DATA_ALIGN(gEncLink_tskStack, 32)
#pragma DATA_SECTION(gEncLink_tskStack, ".bss:taskStackSection")
UInt8 gAdjustEncInputFrameStack[ENC_LINK_OBJ_MAX][DEC_LINK_TSK_STACK_SIZE];
...
Void enc_inputframe_adjust_tsk(struct Utils_TskHndl *pTsk, Utils_MsgHndl * pMsg)
{
	...
	Uint32 count = 0;
	EncLink_Obj *pObj;
	EncLink_ChInputFpsParam params[ENC_LINK_MAX_CH];
	...
	while(1)
	{
		if(count++ == 5){
			count = 0;
//			Vps_printf("video: enc is living!!!!, link id = %d\n", pObj->linkId);
		}
		Task_sleep(2000);
		for(index = 0; index < ENC_LINK_MAX_CH; index++){
			if((pObj->ulHistoryFrameRate[index] != pObj->ulEncFrameRate[index]/2)
				&& pObj->ulEncFrameRate[index])
			{
				pObj->ulHistoryFrameRate[index] = pObj->ulEncFrameRate[index]/2;
 				params[index].chId = index;
				params[index].inputFps = pObj->ulHistoryFrameRate[index];
...
//				Vps_printf("video: chid = %d, inputframerate = %d\n", index,
//								pObj->ulHistoryFrameRate[index]);
			}
			pObj->ulEncFrameRate[index] = 0;
		}
	}
}
...
Int32 EncLink_init()
{
    ...
    for (objId = 0; objId < ENC_LINK_OBJ_MAX; objId++)
    {
		...
		memset(pObj->ulHistoryFrameRate, 60, ENC_LINK_MAX_CH);
		memset(pObj->ulEncFrameRate, 60, ENC_LINK_MAX_CH);
...
#if 1
		memset(ad_name, 0, 32);
		UTILS_SNPRINTF(ad_name, "enc_adjust_infps_%d", objId);
		status= Reach_tskCreate(&gAdjustEncInputFrameTsk[objId],
		                         enc_inputframe_adjust_tsk,
		                         ENC_LINK_TSK_PRI,
		                         gAdjustEncInputFrameStack[objId],
		                         ENC_LINK_TSK_STACK_SIZE, pObj, ad_name);
		UTILS_assert(status == FVID2_SOK);
#endif
    }
