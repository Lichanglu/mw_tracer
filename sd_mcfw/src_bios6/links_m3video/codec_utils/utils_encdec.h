...
typedef enum EncDec_ResolutionClass {
    ...
    UTILS_ENCDEC_RESOLUTION_CLASS_4K,
    UTILS_ENCDEC_RESOLUTION_CLASS_3840x1200,
...
#define UTILS_ENCDEC_RESOLUTION_CLASS_4K_WIDTH                            (3840)
#define UTILS_ENCDEC_RESOLUTION_CLASS_4K_HEIGHT                           (2400)

#define UTILS_ENCDEC_RESOLUTION_CLASS_3840x1200_WIDTH                     (3840)
#define UTILS_ENCDEC_RESOLUTION_CLASS_3840x1200_HEIGHT                    (1200)
...
#define UTILS_ENCDEC_ENCODER_IVACH_BASE       (0)
#define UTILS_ENCDEC_DECODER_IVACH_BASE       (SYSTEM_LINK_ID_VENC_COUNT * ENC_LINK_MAX_CH)
#define UTILS_ENCDEC_IVACHID_INVALID          ~(0u)

/**
    \brief Map the passed linkID and channelID into a unique IVA channel ID

    \param linkID        [IN] System LinkID

    \param channelID     [IN] Channel within the link

    \return Unique IVA channel ID
*/
static inline UInt32 Utils_encdecMapLinkChID2IvaChID(Int linkID, Int channelID)
{
    UInt32 ivaChID = UTILS_ENCDEC_IVACHID_INVALID;

    if ((linkID >= SYSTEM_LINK_ID_VENC_START) &&
        (linkID <= SYSTEM_LINK_ID_VENC_END))
    {
        ivaChID = UTILS_ENCDEC_ENCODER_IVACH_BASE +
            ((linkID - SYSTEM_LINK_ID_VENC_START) * ENC_LINK_MAX_CH) +
            channelID;
    }
    if ((linkID >= SYSTEM_LINK_ID_VDEC_START) &&
        (linkID <= SYSTEM_LINK_ID_VDEC_END))
    {
        ivaChID = UTILS_ENCDEC_DECODER_IVACH_BASE +
            ((linkID - SYSTEM_LINK_ID_VDEC_START) * DEC_LINK_MAX_CH) +
            +channelID;
    }
