...
static Int32 DecLink_codecPopulateOutFrmFormat(DecLink_Obj *pObj, UInt32 chId)
{
	...
	Vps_printf("reslutionClass[%d] width x height [%dx%d]\n",pOutObj->outChObj[chId].reslutionClass,pFormat->width,pFormat->height);
...
static Int32 DecLink_codecMapCh2ResolutionPool(DecLink_Obj * pObj, UInt32 chId)
{
	...
	Vps_printf("ch[%d] targetMaxWidth x targetMaxHeight [%dx%d] \n",chId,pObj->createArgs.chCreateParams[chId].targetMaxWidth,pObj->createArgs.chCreateParams[chId].targetMaxHeight);
    UTILS_assert(status == DEC_LINK_S_SUCCESS);
...
static Int32 DecLink_codecGetProcessedData(DecLink_Obj * pObj)
{
    ...
    {
		test = outFrameList.numFrames + test;

		//Vps_printf("outFrameList[%d]\n",test);
...
Int32 DecLink_printStatistics (DecLink_Obj * pObj, Bool resetAfterPrint)
{
	...
	for (chId = 0; chId < 6; chId++)
    {
		...
		Vps_printf("inQueNum[%d] emptyQueNum[%d]  pro[%d] getpro[%d]\n",Utils_queGetQueuedCount(&pChObj->inQue),\
			Utils_queGetQueuedCount(&pObj->outObj.bufOutQue.emptyQue[pChObj->
                   allocPoolID]),pChObj->processReqestCount,pChObj->getProcessedBufCount);
	}
	
    Vps_printf("fullQueQueNum [%d] processQue[%d] [%d] [%d] processDoneQue[%d] pObj->reqQue[%d] [%d]\n",Utils_queGetQueuedCount(&pObj->outObj.bufOutQue.fullQue),\
		Utils_queGetQueuedCount(&pObj->decProcessTsk[0].processQue),\
		Utils_queGetQueuedCount(&pObj->decProcessTsk[1].processQue),\
		Utils_queGetQueuedCount(&pObj->decProcessTsk[2].processQue),\
		Utils_queGetQueuedCount(&pObj->processDoneQue),\
		Utils_queGetQueuedCount(&pObj->reqQue),\
		pObj->statetest
		);

	Vps_printf("Test out[%d] in[%d]\n",pObj->framesout,pObj->framesin);
