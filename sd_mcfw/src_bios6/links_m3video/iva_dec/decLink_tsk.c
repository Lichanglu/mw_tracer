...
Void DecLink_tskMain(struct Utils_TskHndl *pTsk, Utils_MsgHndl * pMsg)
{
    ...
    Bool ackMsg, done;
    Int32 status, tmpStatus;
    DecLink_Obj *pObj;
    UInt32 flushCmds[2];
    Utils_MsgHndl *pMsgTmp = NULL;
        ...
        switch (cmd)
        {
		  ...
		  gDecLinkRunCount++;
            ...
            case DEC_LINK_CMD_LATE_ACK:
                if (pMsgTmp != NULL)
                {
                    Utils_tskAckOrFreeMsg(pMsgTmp, tmpStatus);
                }
                pMsgTmp = NULL;
                break;
...
{	
    ...
    status = Utils_bufGetFullExt(&pObj->outObj.bufOutQue, pFrameList,
                               BIOS_NO_WAIT);

	pObj->framesout = pFrameList->numFrames + pObj->framesout;
                             ...
                             FVID2_FrameList * pFrameList)
{
    ...
    for (i = 0; i < pFrameList->numFrames;i++)
    {
        ...
        frame = pFrameList->frames[i];
        UTILS_assert(pObj->chObj[frame->channelNum].numQueuedOutputBufs > 0);
        pObj->chObj[frame->channelNum].numQueuedOutputBufs--;
    }
    Hwi_restore(key);
    freeStatus =  DecLink_codecFreeProcessedFrames(pObj, pFrameList);

	pObj->framesin = pFrameList->numFrames + pObj->framesin;
    return (freeStatus);
}
