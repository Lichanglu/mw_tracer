...
/**
* @ ∫Ø ˝√˚≥∆: track_students_right_side_area_check()
* @	∫Ø ˝π¶ƒ‹: …Ë∂®∏˙◊Ÿ«¯”Ú
* @  ‰»Î≤Œ ˝: input_param -- ”¶”√≤„…Ë÷√µƒ≤Œ ˝
* @  ‰≥ˆ≤Œ ˝: Œﬁ
* @ ∑µªÿ÷µ:   Œﬁ
*/
void track_students_double_side_area_check(StuDoubleSideTrackHand *inst)
{
...
* @ ∑µªÿ÷µ:   Œﬁ
*/
static int32_t	draw_point(StuDoubleSideTrackHand *inst,int8_t *buffer, yuv_value_info_t *yuv_value,
...
* @ ∑µªÿ÷µ:   Œﬁ
*/
static int32_t	draw_line(StuDoubleSideTrackHand *inst,int8_t *buffer,yuv_value_info_t *yuv_value,
...
* @ ∑µªÿ÷µ:	  Œﬁ
*/
static void draw_rectangle(StuDoubleSideTrackHand *inst,int8_t *buffer,yuv_value_info_t *yuv_value,
	line_info_t *line_info)
{
    int32_t i,j;
...
* @ ∑µªÿ÷µ:	  Œﬁ
*/
static void fill_rectangle(StuDoubleSideTrackHand *inst,int8_t *buffer,yuv_value_info_t *yuv_value,
	line_info_t *line_info)
{
	int nSrcBufWidth=inst->t_global_obj.nSrcBufWidth;
	int nSrcBufHeight=inst->t_global_obj.nSrcBufHeight;
...
* @ ∑µªÿ÷µ:	  Œﬁ
*/
static void draw_track_rect(StuDoubleSideTrackHand *inst,int8_t *buffer)
{
	StuDoubleSideITRACK_Params *input_param=&(inst->input_param);
	...
	if(input_param->dynamic_param.message == 1)
	{
...
static int32_t move_check_data_copy(int8_t *output_buf, StuDoubleSideTrackHand *inst)
{
	StuDoubleSideITRACK_Params *input_param=&(inst->input_param);
...
static int32_t check_track_point(int8_t *output_buf, StuDoubleSideTrackHand *inst)
{
	StuDoubleSideITRACK_Params 	*input_param	= &(inst->input_param);
...
//	uint8_t *src_y_buf		= inst->t_global_obj.src_y_buf; //¥Ê∑≈“™Ω¯––‘ÀÀ„µƒ‘¥ybuf ˝æ›	
	...
	if(input_param->dynamic_param.refreshType == 0)
	{
		UpdateBackground(inst->pBackGroundModel,Area_vertex,output_buf,output_buf + offset ,nSrcBufWidth,gray_buf,video_width,sens,4,input_param->dynamic_param.refreshTime*22);
		return 0;
	}
...
int32_t studoubleside_set_check_max_rect(StuDoubleSideTrackHand *inst)
{
	StuDoubleSideITRACK_Params 	*input_param	= &(inst->input_param);
	int32_t		index	= 0;
	int32_t		min_x	= 704;
...
int32_t studoubleside_get_max_rect(StuDoubleSideITRACK_Params *pParams, T_Rect *prect)
{
	StuDoubleSideITRACK_Params 	*input_param	= pParams;
	int32_t		index	= 0;
	int32_t		min_x	= 704;
	int32_t		min_y	= 576;
	int32_t		max_x	= 0;
...
static int32_t gray_close(StuDoubleSideTrackHand *inst)
{
	StuDoubleSideITRACK_Params 	*input_param	= &(inst->input_param);
	uint8_t *Area_vertex	= inst->t_global_obj.Area_vertex;
...
static int32_t find_block_kernel(StuDoubleSideTrackHand *inst)
{
	StuDoubleSideITRACK_Params 	*input_param			= &(inst->input_param);
	uint8_t 		*Area_vertex			= inst->t_global_obj.Area_vertex;
	uint8_t 		*pGray					= inst->t_global_obj.gray_buf;
...
//	Vps_printf(">>>>>>>>>>%d %d %d %d\n",left,right,top,bottom);
	...
	for(i=0;i<=rect_count;i++)
	{
		...
		//if(input_param->dynamic_param.message == 0)
...
static Int32 Draw(StuDoubleSideTrackHand *inst,int8_t *input_buf, int8_t *output_buf)
{
...
static Int32 Draw_1(StuDoubleSideTrackHand *inst,int8_t *input_buf, int8_t *output_buf)
{
...
//ª≠UV
static Int32 Draw_2(StuDoubleSideTrackHand *inst,int8_t *input_buf, int8_t *output_buf)
{
...
//ª≠UV
static Int32 Draw_3(StuDoubleSideTrackHand *inst,int8_t *input_buf, int8_t *output_buf, int8_t *uv)
{
...
static int ClearRect(StuDoubleSideTrackHand *inst,int nTriggerWidth,StuDoubleSideITRACK_OutArgs *output_param, int8_t *output_buf)
{
...
int32_t	track_students_double_side_process(StuDoubleSideTrackHand *inst,int8_t *input_buf, int8_t *output_buf, 
	StuDoubleSideITRACK_OutArgs *output_param)
{
	StuDoubleSideITRACK_Params 	*input_param 	= &(inst->input_param);
...
//	return 0;
	...
	else if(output_param->trigger_num == 1)
	{
		...
		line_info.start_x	= inst->t_global_obj.g_out_rect[0].nLeft;
		line_info.start_y	= inst->t_global_obj.g_out_rect[0].nTop;
		line_info.end_x 	= inst->t_global_obj.g_out_rect[0].nRight;
		line_info.end_y 	= inst->t_global_obj.g_out_rect[0].nBottom;
		
		draw_rectangle(inst,output_buf, &yuv_value, &line_info);
		
		output_param->cmd_type = 1;
	}
	else if(output_param->trigger_num == 2)
	{
		if(inst->t_global_obj.g_out_rect[1].nLeft >= inst->t_global_obj.g_out_rect[0].nLeft)
		{
			maxindex = 1;
			minindex = 0;
		}
		...
		space_value01 = inst->t_global_obj.g_out_rect[maxindex].nLeft - inst->t_global_obj.g_out_rect[minindex].nRight;
		width0	= inst->t_global_obj.g_out_rect[minindex].nRight - inst->t_global_obj.g_out_rect[minindex].nLeft;
		width1	= inst->t_global_obj.g_out_rect[maxindex].nRight - inst->t_global_obj.g_out_rect[maxindex].nLeft;
		
		space_value01 = abs(space_value01);
		width0 = abs(width0);
		width1 = abs(width1);

		//Vps_printf("width0 = %d width1 =%d space_value01 =%d l %d r %d l %d r %d\n",width0,width1,space_value01,inst->t_global_obj.g_out_rect[minindex].nLeft,inst->t_global_obj.g_out_rect[minindex].nRight,inst->t_global_obj.g_out_rect[maxindex].nLeft,inst->t_global_obj.g_out_rect[maxindex].nRight);
		if((space_value01 > width0) && (space_value01 > width1))
		{
			output_param->cmd_type = 2;
		}
		else
		{
			...
			output_param->trigger[0].x = (inst->t_global_obj.g_out_rect[minindex].nLeft + inst->t_global_obj.g_out_rect[maxindex].nRight)/2;
			output_param->trigger[0].y = (inst->t_global_obj.g_out_rect[minindex].nTop + inst->t_global_obj.g_out_rect[maxindex].nBottom)/2;

			yuv_value.y_value = 76;
			yuv_value.cr_value = 85;
			yuv_value.cb_value = 255;

			line_info.start_x 	= inst->t_global_obj.g_out_rect[minindex].nLeft;
			line_info.start_y 	= inst->t_global_obj.g_out_rect[minindex].nTop;
			line_info.end_x 	= inst->t_global_obj.g_out_rect[maxindex].nRight;
			line_info.end_y		= inst->t_global_obj.g_out_rect[maxindex].nBottom;
	...
	else if(output_param->trigger_num == 3)
	{
		int max,min,i,maxindex,minindex,midindex= 0;
		min = 65535;
		for(i = 0; i < 3; i++)
		{
			if(inst->t_global_obj.g_out_rect[i].nLeft > max)
			{
				max = inst->t_global_obj.g_out_rect[i].nLeft;
				maxindex = i;
			}

			if(inst->t_global_obj.g_out_rect[i].nLeft < min)
			{
				min = inst->t_global_obj.g_out_rect[i].nLeft;
				minindex = i;
			}
		}
		...
		width0	= inst->t_global_obj.g_out_rect[maxindex].nRight - inst->t_global_obj.g_out_rect[maxindex].nLeft;
		width1	= inst->t_global_obj.g_out_rect[midindex].nRight - inst->t_global_obj.g_out_rect[midindex].nLeft;
		width2	= inst->t_global_obj.g_out_rect[minindex].nRight - inst->t_global_obj.g_out_rect[minindex].nLeft;

		space_value01 = inst->t_global_obj.g_out_rect[midindex].nLeft - inst->t_global_obj.g_out_rect[minindex].nRight;
		space_value12 = inst->t_global_obj.g_out_rect[maxindex].nLeft - inst->t_global_obj.g_out_rect[midindex].nRight;
		space_value01 = abs(space_value01);
		space_value12 = abs(space_value12);
		...
		if(((space_value01 > width0) && (space_value01 > width1))
			|| ((space_value12 > width1) && (space_value12 > width2)))
		{
			output_param->cmd_type = 2;
		}
		else
	...
	{
		inst->t_global_obj.frame_num ++;
		if(inst->t_global_obj.frame_num > 13)
		{
