...
//    if(pScdAlgLinkObj->createArgs.chDefaultParams[chId].mode & SCD_DETECTMODE_MONITOR_BLOCKS)
    ...
    if(pScdAlgLinkObj->chParams[chId].mode & SCD_DETECTMODE_MONITOR_BLOCKS)
    {
		...
		//if(scdResultBuff->blkResult[blkIdx].numPixelsChanged > 5)
			//Vps_printf("threshold:[%d] numPixelsChanged:[%d] blkHeight:[%d] sensitivity:[%d]\n",
			//		threshold, scdResultBuff->blkResult[blkIdx].numPixelsChanged, blkHeight, blockConfig->sensitivity);
                if(scdResultBuff->blkResult[blkIdx].numPixelsChanged > threshold)
                {
			//Vps_printf(" xxx threshold:[%d] numPixelsChanged:[%d]\n", threshold, scdResultBuff->blkResult[blkIdx].numPixelsChanged);
                    numBlkChg++;
                }
            }
        }
                            ...
                            AlgLink_ScdChblkUpdate * params)
{
	...
	//Vps_printf("[AlgLink_ScdalgSetChblkUpdate][%d] monitored:[%d] sensitivity:[%d]\n",
	//			idx,  blkConfig->monitored, blkConfig->sensitivity);
        channelUpdated = 0;
    }
